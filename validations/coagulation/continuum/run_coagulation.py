...
def run_coagulation(particle_dynamics_model_type):
    ...
    init_sec = 36;
    spacing_factor = 1.5;
    mass_factor = spacing_factor ** init_sec;
    ...
    if particle_dynamics_model_type == "Monodisperse":
        N_agg = N_0;
        N_pri = N_0;
        C_tot = N_0 * constants.incepient_soot_C * mass_factor;
        H_tot = N_0 * constants.incepient_soot_H * mass_factor;
        soot_array = np.hstack((N_agg, N_pri, C_tot, H_tot));
    elif particle_dynamics_model_type == "Sectional":
        particle_dynamics_model.number_of_sections = n_secs;
        particle_dynamics_model.spacing_factor = spacing_factor;
        ...
        for sec in range(0, n_secs):
            if sec == init_sec:
                N_agg_sec[sec] = N_0;
                N_pri_sec[sec] = N_0;
                H_tot_sec[sec] = N_0 * constants.incepient_soot_H * mass_factor / constants.Av; 
            else:
    ...
    particle_dynamics_model.beta_interp_method_type = "continuum";
    ...
    extra_sectional_headers = ["d_mg[nm]", "sigma_mg", "d_gg[nm]", "sigma_gg"];
    ...
    def calc_sigma_gg(N_agg_sec, d_g_sec):
        # d_m_sec = sectional_data_list["d_m"][-1];
        # N_agg_sec = sectional_data_list["N_agg"][-1];
        d_gg = np.exp(
            np.sum(np.log(d_g_sec)*N_agg_sec)/np.sum(N_agg_sec)
        );
        log_sigma_cr = (
            (
                np.sum(
                    N_agg_sec * 
                    (
                        (np.log(d_g_sec/d_gg))**2.0
                    )
                )/(np.sum(N_agg_sec))
            ) ** 0.5
        );
        return d_gg, np.exp(log_sigma_cr);
    ...
    def append_sectional():
        ...
        d_gg, sigma_gg = calc_sigma_gg(np.array(sectional_data_list["N_agg"][-1]), np.array(sectional_data_list["d_g"][-1]));
        sectional_data_list["sigma_gg"].append(sigma_gg);
        sectional_data_list["d_gg[nm]"].append(d_gg*1e9);
