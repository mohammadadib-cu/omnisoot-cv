...
def plot():
    ...
    for PAH_growth_model in PAH_growth_models:
        for particle_dynamics_model in particle_dynamics_models:
            results_df[PAH_growth_model][particle_dynamics_model] = pd.read_csv(
                f"{results_dir}/{particle_dynamics_model}/{PAH_growth_model}/results.csv"
            );
    ...
    for PAH_growth_model_index, PAH_growth_model in enumerate(PAH_growth_models):
        ...
        for particle_dynamics_model_index, case in enumerate(particle_dynamics_models):
            ...
            # Carbon Flux
            t_data = df["t[s]"].to_numpy();        
            z_data = df["C_mass_error"].to_numpy();
            ...
            z_data = df["H_mass_error"].to_numpy();
            ax.plot(t_data, z_data,
            ...
            z_data = df["enthalpy_error"].to_numpy();
        ...
        if PAH_growth_model_index == 2:
            ...
            ax.text(0.41 + x_offset, 0.35 + y_offset, 'MPBM', transform=ax.transAxes, fontsize = 17);
            ax.text(0.58 + x_offset, 0.35 + y_offset, 'SPBM', transform=ax.transAxes, fontsize = 17);
            

        
        ax.text(0.25, 0.82, labels[PAH_growth_model], transform=ax.transAxes, fontsize = 22);
        
    save_plot("relerr_psr", postprocess_dir = postprocess_dir)
