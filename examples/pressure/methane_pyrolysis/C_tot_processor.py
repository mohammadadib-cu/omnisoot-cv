...
def main():
    ...
    for file_path in list(files_paths.glob("**/*.csv")):
        ...
        for i in range(1, len(t_array)):
            C_tot_int[i] = simpson(y = C_tot_rate[:(i+1)], x = t_array[:(i+1)]);
            C_tot_inception_int[i] = simpson(y = C_tot_inception[:(i+1)], x = t_array[:(i+1)]);
            C_tot_PAH_int[i] = simpson(y = C_tot_PAH[:(i+1)], x = t_array[:(i+1)]);
            C_tot_growth_int[i] = simpson(y = C_tot_growth[:(i+1)], x = t_array[:(i+1)]);
        
        df["C_tot_int[mol]"] = C_tot_int;
        df["C_tot_inception_int[mol]"] = C_tot_inception_int;
        df["C_tot_PAH_int[mol]"] = C_tot_PAH_int;
        df["C_tot_growth_int[mol]"] = C_tot_growth_int;
